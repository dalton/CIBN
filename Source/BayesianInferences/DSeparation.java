...
public class DSeparation {
   ...
   */
  public DSeparation(BayesNet b_n) {
    bn = b_n;
  }
   ...
   * variable index_to.                                        
   */
  public boolean adj(int index_from, int index_to, int flag) {
  ...
  public boolean adj_st(int index_from, int index_to) {
	    ProbabilityFunction pf = null;

	   // System.out.println("ADJ_ST");

	        for (int i=0; i<bn.number_probability_functions(); i++) {
	            if (bn.get_probability_function(i).get_index(0) == index_to) {
	                pf = bn.get_probability_function(i);
	               // System.out.println("pf_to = " + pf.get_variables()[0].get_name());
	               // System.out.println("pf_to = " + pf.number_variables());
	                break;
	            }
	        }
	        if (pf == null) return(false);
	        ...
	        for (int i=1; i<pf.number_variables(); i++) {
	          if (pf.get_index(i) == index_from){
	        	  //System.out.println("pf_from = " + pf.get_variables()[i].get_name());
	        	  	return(true);
	          }
	        }
