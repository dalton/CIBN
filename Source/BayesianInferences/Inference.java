...
public class Inference {
  ...
  private boolean do_produce_clusters;
  ...
  protected void inference(String queried_variable_name) {
    if (isDo_produce_clusters()) { // If clusters are generated:
        ...
        if (index_queried != BayesNet.INVALID_INDEX) { // If the queried_variable_name is valid:
          ...
          if ((buck == null) || (buck.cluster == null)) { 
           ...
           // System.out.println("if ((buck == null) || (buck.cluster == null))");
          }
  ...
  private void inference(Ordering or) {
    ...
    bucket_tree =  new BucketTree(or, isDo_produce_clusters());
    // Add the new BucketTree to the bucket_forest and update bucket_for_variable.
    if (isDo_produce_clusters()) add_bucket_tree();
  ...
  public boolean areClustersProduced() {
    return(isDo_produce_clusters());
  }

/**
 * @param do_produce_clusters the do_produce_clusters to set
 */
public void setDo_produce_clusters(boolean do_produce_clusters) {
	this.do_produce_clusters = do_produce_clusters;
}

/**
 * @return the do_produce_clusters
 */
public boolean isDo_produce_clusters() {
