...
public class BayesNet {
  ...
  public ProbabilityFunction get_function(ProbabilityVariable p_v) {
	 // System.out.println("probability_functions.length= "+probability_functions.length);
    for (int i=0; i<probability_functions.length; i++){
...
/**
 * Get the number variables in the S set.
 */
  public int number_s() {
  ...
  /**
   * Get the number variables in the C set.
   */
    public int number_c() {
	...
	/**
	 * Get the number variables in the T set.
	 */
  public int number_t() {
	int t=0;  
	if (probability_variables == null) return(BayesNet.INVALID_INDEX);
  ...
  public int getpvIdx(ProbabilityVariable A){
	  int idx=-1;
	  for (int i=0; i<probability_variables.length; i++){
		  if(A.get_name().equals(probability_variables[i].get_name()))
			  return i;
	  }
	  return idx;
  }
  public boolean isEdge(ProbabilityVariable A, ProbabilityVariable B){
	  int a_idx = getpvIdx(A);
	  int b_idx = getpvIdx(B);
	  ProbabilityFunction a = get_probability_function(a_idx);
	  ProbabilityFunction b = get_probability_function(b_idx);
	  int a_num = a.number_variables();
	  int b_num = b.number_variables();
	  DiscreteVariable[] aDV = new DiscreteVariable[a_num];
	  DiscreteVariable[] bDV = new DiscreteVariable[b_num];
	  aDV = a.get_variables();
	  bDV = b.get_variables();
	  for (int i=1; i<a_num; i++){
		  if(aDV[i].get_name().equals(B.get_name()))
			  return true;
	  }
	  for (int i=1; i<b_num; i++){
		  if(bDV[i].get_name().equals(A.get_name()))
			  return true;
	  }
	  return false;
  }
  public boolean isBiEdge(ProbabilityVariable A, ProbabilityVariable B){
	  int a_idx = getpvIdx(A);
	  int b_idx = getpvIdx(B);
	  ProbabilityFunction a = get_probability_function(a_idx);
	  ProbabilityFunction b = get_probability_function(b_idx);
	  int a_num = a.number_ajk();
	 // int b_num = b.number_variables();
	  DiscreteVariable[] aDV = new DiscreteVariable[a_num];
	 // DiscreteVariable[] bDV = new DiscreteVariable[b_num];
	  aDV = a.getAjk();
