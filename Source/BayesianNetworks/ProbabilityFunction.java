...
public class ProbabilityFunction extends DiscreteFunction {
  ...
  public ProbabilityFunction(BayesNet b_n, int n_vb, int n_vl,int n_ajk, Vector prop) {
	    super(n_vb, n_vl, n_ajk);
	    properties = prop;
	    bn = b_n;
	  }
  /**
  ...
  public ProbabilityFunction(BayesNet b_n, DiscreteVariable pvs[],
		     double v[],DiscreteVariable a_jk[], Vector prop) {
	  super(pvs, v, a_jk);
	  properties = prop;
	  bn = b_n;
  }
  ...
  public ProbabilityFunction(DiscreteFunction df, double[] new_values) {
    super(df.variables, new_values, df.ajk);
  ...
  public ProbabilityFunction(DiscreteFunction df, BayesNet b_n) {
    super(df.variables, df.values, df.ajk);
    bn = b_n;    
    if (df instanceof ProbabilityFunction) 
        properties = ((ProbabilityFunction)df).properties;
  }
  ...
  public void save_xml_0_3(PrintStream out) {
    ...
    /*
    if (ajk != null) {
        out.println("\t<FOR>" + ajk[0].name + "</FOR>");
        for (j=1; j<ajk.length; j++) {
  	    out.println("\t<GIVEN>" + ajk[j].name + "</GIVEN>");
        }
      }
      */
  ...
  /*  
// only work for xml file.
    out.print("{");
    out.println();
    if(ajk != null){
        
    	  out.print("Bidirected edge ( ");
    	  for (j=0; j<ajk.length; j++) {
    		  out.print( " \"" + ajk[j].name + "\" ");
    	  }
    	   out.print(")");
    }
  	  out.print("}");
  ...
  */
  ...
  public void print_ajk(PrintStream out) {
	  if(ajk != null){
	        
    	  out.print("Bidirected edge ( ");
    	  for (int j=0; j<ajk.length; j++) {
    		  out.print( " \"" + ajk[j].name + "\" ");
    	  }
    	   out.println(")");
    }
