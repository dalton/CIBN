...
public class ProbabilityVariable extends DiscreteVariable {
  ...
  protected int s_index = BayesNet.S_INDEX;
  protected int t_index = BayesNet.T_INDEX;
  protected int c_index = BayesNet.C_INDEX;
  ...
  final static String s_property_name = "S";
  final static String t_property_name = "T";
  final static String c_property_name = "C";
  ...
  public void save_xml_0_3(PrintStream out) {
    ...
    if (is_S())
        out.println("\t<PROPERTY>" + s_property_name + " " +
                    values[s_index] + "</PROPERTY>");
    
    if (is_T())
        out.println("\t<PROPERTY>" + t_property_name + " " +
                    values[t_index] + "</PROPERTY>");
    if (is_C())
        out.println("\t<PROPERTY>" + c_property_name + " " +
                    values[c_index] + "</PROPERTY>");
  ...
  public void print(PrintStream out) {
    ...
    if (name != null)
      out.print(" \"" + name + "\" ");
    /*if(is_S())
    	out.print("\tis in S set.");
    if(is_T())
      	out.print("\tis in T set.");
      	*/
    out.print("{");
    if (values != null) {
  ...
  /**
   * Indicate whether the current ProbabilityVariable is in
   * S set or not.
   */
  public boolean is_S() {
	    return(s_index != BayesNet.S_INDEX);
	  }
  /**
   * Indicate whether the current ProbabilityVariable is in
   * T set or not.
   */
  public boolean is_T() {
		    return(t_index != BayesNet.T_INDEX);
	  }
  /**
   * Indicate whether the current ProbabilityVariable is in
   * C set or not.
   */
  public boolean is_C() {
		    return(c_index != BayesNet.C_INDEX);
	  }
  /**
  ...
  /**
   * Set a value of the current ProbabilityVariable as S.
   * @param v isS value.
   */
  public void set_isS_value(String v) {
    int index = index_of_value(v);
    //System.out.println("s_index = "+index);
    if (index == BayesNet.S_INDEX) return;
    s_index = index;
    observed_index = index;
   // set_observed_value(v);
  }  /**
   * Set a value of the current ProbabilityVariable as T.
   * @param v isT value.
   */
  public void set_isT_value(String v) {
    int index = index_of_value(v);
    if (index == BayesNet.T_INDEX) return;
    t_index = index;
    observed_index = index;
  }
  /* Set a value of the current ProbabilityVariable as T.
  * @param v isC value.
  */
 public void set_isC_value(String v) {
   int index = index_of_value(v);
   if (index == BayesNet.C_INDEX) return;
   c_index = index;
   observed_index = index;
 }
  /** 
  ...
  /**
   * Get the index of the isS value.
   */
  public int get_isS_index() {
    return(s_index);
  }
  /**
   * Get the index of the isT value.
   */
  public int get_isT_index() {
    return(t_index);
  }
  /**
   * Get the index of the isC value.
   */
  public int get_isC_index() {
    return(c_index);
  }
  /**
  ...
  /**
   * Set the ProbabilityVariable as observed..
   */
  public void set__observed_index() {
    observed_index = 0;
  }
  /**
   * Set the ProbabilityVariable as not observed..
   */
  public void set_invalid_isS_index() {
    s_index = BayesNet.S_INDEX;
  }
  /**
   * Set the ProbabilityVariable as not observed..
   */
  public void set_invalid_isT_index() {
    t_index = BayesNet.T_INDEX;
  }
  /**
   * Set the ProbabilityVariable as not observed..
   */
  public void set_invalid_isC_index() {
    c_index = BayesNet.C_INDEX;
  }
