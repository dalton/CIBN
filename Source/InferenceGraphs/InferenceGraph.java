...
public class InferenceGraph {
  ...
  public final static int IDENTIFIABILITY = 6;
  public static final int ALL_OBSERVED = 7;
  public final static int IDENTIFY_H = 8;
  public final static int IDENTIFY_T = 9;
  public final static int Verma_T = 10;
  public final static int Build_SEMs = 11;
...
System.out.println("convert_bayes_net   qbn.number_variables()=" +qbn.number_variables());
  ...
  QuasiBayesNet convert_graph() {
    ...
    /*    if (node.pf.ajk!=null){
        	System.out.println("node="+node.pv.get_name());
        	for(int j=0; j<node.pf.number_ajk(); j++)
        		System.out.println("	Ajk ="+node.pf.getAjk()[j].get_name());
        }
        */
    }
  ...
  /**
   * print the analysis result of identifiability
   * @param out
   */
  public void print_identifiability_analysis(PrintStream out){
	if (qbn.number_s()<=0 )
		out.print("Please set S and T before use InferenceGraph.print_identifiability_analysis")  ;
	else{
		//call function for identifiable
		//qbn.print(out);
		AlgorithmComputing identifiability = new AlgorithmComputing(qbn);
		identifiability.print(out);
		//out.print("Yes Identifiable!!");
	}
  }
  public void print_uai04_analysis(PrintStream out){
		if (qbn.number_s()<=0 )
			out.print("Please set S, C and T before use InferenceGraph.print_uai04_analysis")  ;
		else{
			//call function for identifiable
			//qbn.print(out);
  ...
  public void print_r305_analysis(PrintStream out){
		//common out if at 05/26/2008
	  //if (qbn.number_s()<=0 )
		//	out.print("Please set S before use InferenceGraph.print_r305_analysis")  ;
		//else{
			//call function for identifiable
			//qbn.print(out);
			//Algorithm1 uai04 = new Algorithm1(qbn);
			Verma_Type_Functional_Constraint VT = new Verma_Type_Functional_Constraint();
			VT.print(out, qbn);
			//out.print("Yes Identifiable!!");
		//}
	  }
  public void print_buildSEMs_analysis(PrintStream out, String queried_variable){
		
			//call function for identifiable
			//qbn.print(out);
			//Algorithm1 uai04 = new Algorithm1(qbn);
			BuildSEMs build = new BuildSEMs();
			build.build_sems(out, qbn, queried_variable);
			//out.print("Yes Identifiable!!");
 ...
 /**
  ...
  InferenceGraphNode child) {
    ...
    // object updated.
    System.out.println("child = "+child.pv.get_name());
    child.init_dists();
  ...
  //Parent is similar to ajk_one, Child is similar to ajk_two.
  public boolean create_ajk_arc(InferenceGraphNode ajk_one,
		  InferenceGraphNode ajk_two) {
		    // Check whether the given parent is already a parent of the
		    // given child.
		    for (Enumeration e = ajk_two.ajk.elements(); e.hasMoreElements(); ) {
		        if ( ajk_one == ( (InferenceGraphNode)(e.nextElement()) ) )
		            return(false);
		    }
		    
		    // First put child into the children of parent
		    ajk_one.ajk.addElement(ajk_two);
		    // Second put parent into the parents of child
		    ajk_two.ajk.addElement(ajk_one);

		    // The parent is not further affected by the arc.
		    // The child must have its ProbabilityFunction
		    // object updated.
		    ajk_one.init_ajk_dists();
		    ajk_two.init_ajk_dists();
  ...
  public void delete_ajk_arc(InferenceGraphNode parent,
		  InferenceGraphNode child) {
		    // First remove child into the children of parent
		    parent.ajk.removeElement(child);
		    // Second remove parent into the parents of child
		    child.ajk.removeElement(parent);

		    // The parent is not further affected by the arc.
		    // The child must have its ProbabilityFunction
		    // object updated.
		    child.init_ajk_dists();
...
public void print_set_all_observed(PrintStream pstream) {
	// TODO Auto-generated method stub
	st.set_all_observed(pstream);
}
}
