...
public class InferenceGraphNode {
    ...
    Vector ajk = new Vector();
    //Vector ajk_2 = new Vector();
   ...
   Point position) {
    ...
    void init_dists() {
        ...
        for (i = 1, e = parents.elements(); e.hasMoreElements(); i++) {
            ...
            pvs[i] = pnode.pv;
            //System.out.println("pnode = "+ pvs[i].get_name()); 
    ...
    void init_ajk_dists() {
        ...
        // Create the probability_variables
        ProbabilityVariable pvs[] =
            new ProbabilityVariable[ajk.size() + 1];
        pvs[0] = pv;
        System.out.println("pv_ajk = "+ pv.get_name());
       // total_values = pv.number_values();
       // new_value = 1.0/((double)(total_values));

        for (i = 1, e = ajk.elements(); e.hasMoreElements(); i++) {
            pnode = (InferenceGraphNode)(e.nextElement());
            pvs[i] = pnode.pv;
            System.out.println("pnode_ajk = "+ pvs[i].get_name());
           // total_values *= pnode.pv.number_values();
        }
	   ...
	   // for (i=0; i < dists.length; i++)
        ...
        // Construct the ProbabilityFunction
        pf = new ProbabilityFunction(defaultInferenceGraphNodeBayesNet,
                                     pf.get_variables(), pf.get_values(),pvs,
                                     defaultInferenceGraphNodeProperties);
	}
...
//    }
    ...
    /**
     * Indicate whether the variable in the node is in S set.
     */
    public boolean is_S() {
    	return(pv.is_S());
    }
    /**
     * Indicate whether the variable in the node is in T set.
     */
    public boolean is_T() {
    	return(pv.is_T());
    }
    ...
    /**
     * Return the isS value for the variable in the node.
     */
    public int get_isS_value() {
        return(pv.get_isS_index());
    }
    /**
     * Return the isT value for the variable in the node.
     */
    public int get_isT_value() {
        return(pv.get_isT_index());
    }
    /**
  ...
  /**
   * Set to S for the node.
   */
  public void set_isS_value(String value) {
        pv.set_isS_value(value);
  }
  /**
   * Set to T for the node.
   */
  public void set_isT_value(String value) {
        pv.set_isT_value(value);
  }
  /**
   * Set to T for the node.
   */
  public void set_isC_value(String value) {
        pv.set_isC_value(value);
  }
  /**
   ...
   * Clear the observation for the node.
   */
  public void clear_isS() {
        pv.set_invalid_isS_index();
  }
  /**
   * Clear the observation for the node.
   */
  public void clear_isT() {
        pv.set_invalid_isT_index();
  }
  /**
   * Clear the observation for the node.
   */
  public void clear_isC() {
        pv.set_invalid_isC_index();
  }
  /**
...
public Vector getAjk() {
	return ajk;
}

public void setAjk_1(Vector ajk) {
	this.ajk = ajk;
}
/*
public Vector getAjk_2() {
	return ajk_2;
}

public void setAjk_2(Vector ajk_2) {
	this.ajk_2 = ajk_2;
}
*/

public ProbabilityVariable getPv() {
	return pv;
}

public void setPv(ProbabilityVariable pv) {
	this.pv = pv;
}

public ProbabilityFunction getPf() {
	return pf;
}

public void setPf(ProbabilityFunction pf) {
	this.pf = pf;
}
