...
public class Verma_Type_Functional_Constraint {
	...
	/*
	 * If A is a effective_parents of S or S itself return true, otherwise
	 * return false
	 */
	if (S == null) {
	    return false;
	...
	/*
	 * Method M = new Method(); M.print_Vector(out, R);
	 * ProbabilityVariable[] re = new ProbabilityVariable[R.size()]; methods
	 * m= new methods(); re = m.from_Vector_to_ProbabilityVariable(R);
	 * M.print_ProbabilityVariableArray(out, re);
	 */
	return R;
    }
	...
	/*
	 * return DUP(S) 1. hidden node. 2 a directed path and internal nodes
	 * are hidden nodes
	 */
	if (S == null) {
	    return false;
	...
	/*
	 * System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@@@@@"); for (int k=0;
	 * k<new_qbn.number_variables(); k++){ if
	 * (new_qbn.get_probability_variable(k).is_observed())
...
/*
	 * Method M=new Method(); System.out.println();
	 * System.out.println("SubBayesNet: "); for (int k=0;
	 * k<Gc.number_variables(); k++){ if
	 * (Gc.get_probability_variable(k).is_observed())
    ...
    /*
     * public void ancestors(QuasiBayesNet qbn,ProbabilityVariable S,
     * Vector<ProbabilityVariable> R){ /*return C's Ancestors and itself to R
     */
    /*
     * if (S==null){ System.out.println("null"); } ProbabilityFunction pfS =
     * qbn.get_function(S); if(pfS!=null){ int n= pfS.number_variables();
     * //Vector R = new Vector(); //if(n>1){ R.add(S); //} methods m = new
     * methods(); DiscreteVariable[] S_Parents= new DiscreteVariable[n-1];
     * S_Parents = m.Subtraction(pfS.get_variables(), S); int len =
     * qbn.number_variables(); ProbabilityVariable[] V = new
     * ProbabilityVariable[len]; V = qbn.get_probability_variables();
     * ProbabilityVariable[] S_parents= new ProbabilityVariable[n-1];
     *
     * boolean flag2 = false; for (int i=0; i<n-1; i++){ flag2 = false; for(int
     * j=0; j<len && !flag2; j++){ if
     * (V[j].get_name().equals(S_Parents[i].get_name())){ S_parents[i] = V[j];
     * flag2 = true; } }
     *
     * }
     */
    /*
     * for (int i=1; i<n; i++){ S_parents[i-1] =
     * qbn.get_probability_variable(pfS.get_variables()[i].get_index());
     *
     * System.out.println(S_parents[i-1].get_name());
     * //System.out.println("R.size="+R.size()); if(!is_in(S_parents[i-1], R)&&
     * S_parents[i-1]!=null){ //R.add(S_parents[i-1]); ancestors(qbn,
     * S_parents[i-1],R); }
     *
     * }
     */
	...
	for (int i = 0; i < len; i++) {
		...
		if (num_c > 1) {
			...
			while (flag1) {
			    ...
			    /*
			     * each descendent set D in G(Si), that not contain
			     * Vi
			     */
	...
	DUP = m.from_Vector_to_ProbabilityVariable(Dup);
	/*
	 * M.print_ProbabilityVariableArray(out, DUP); ProbabilityVariable[]
	 * Union = new ProbabilityVariable[Dup.size()+S.length]; Union =
	 * m.Add(DUP, S); BayesNet Gc = new
	 * BayesNet(SubBayesNet(qbn,Union,"Gc")); QuasiBayesNet new_qbn = new
	 * QuasiBayesNet(Gc); //new_qbn.print(out); /*for (int i=0;
	 * i<new_qbn.number_variables();i++){ if
	 * (new_qbn.get_probability_functions()[i]!=null)
